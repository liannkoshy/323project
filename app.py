...
def prepare_risk_classification(df):
    ...
    if target_col and df[target_col].nunique() >= 2 and len(df) >= 20:
        ...
        # Random Forest as selected model.
        ...
        # Decision Tree model from notebook
...
def prepare_anomaly_review(full_df, alerts_df):
    """Use model output output files for the Anomaly Detector tab."""
...
def prepare_duplicate_ml(df):
    ...
    # Finalv2 duplicate notebook model:
    # Dataset: lapiz_group_duplicate_customer_dataset_v3(ml_features_ready).csv
    # Target: is_duplicate
    # Features: raw_name_similarity, cleaned_name_similarity, same_trn, same_phone, same_organization
    required_features = [
        "raw_name_similarity",
    ...
    if missing_features:
        st.warning("Duplicate dataset is missing required ML features: " + ", ".join(missing_features))
        return df, None, None

    if target_col not in df.columns:
        st.warning("Duplicate dataset is missing target column: is_duplicate")
        return df, None, None
    ...
    if y.nunique() >= 2 and len(df) >= 10:
        ...
        metrics = {
            ...
            "decision_tree_precision": precision_score(y_test, y_pred_dt, zero_division=0),
            "decision_tree_recall": recall_score(y_test, y_pred_dt, zero_division=0),
            "decision_tree_f1": f1_score(y_test, y_pred_dt, zero_division=0),
        }

        model = rf_model
        df["duplicate_probability"] = rf_model.predict_proba(X)[:, 1]
        df["ml_duplicate_prediction"] = rf_model.predict(X)
    else:
...
def prepare_anomaly_ml(df):
    ...
    # Finalv5 anomaly notebook model:
    # Dataset: person5_anomaly_detection_dataset_v2_clean.csv
    # Features: difference, percentage_difference, absolute_percentage_difference
...
duplicate_raw = load_csv("lapiz_group_duplicate_customer_dataset_v3(ml_features_ready).csv")
duplicate_risk_raw = pd.DataFrame()
...
with tabs[8]:
    ...
    with ml_tab1:
        ...
        if duplicate_ml.empty:
            ...
            show_cols = [
                "pair_id", "name_similarity_score", "raw_name_similarity",
                "cleaned_name_similarity", "same_trn", "same_phone",
                "same_email_domain", "same_organization", "hard_negative",
                "is_duplicate", "recommended_split",
                "duplicate_probability", "ml_duplicate_prediction", "review_status"
            ]
            ...
            if not review.empty and "ml_duplicate_prediction" in review.columns:
                chart = review["ml_duplicate_prediction"].value_counts().reset_index()
                chart.columns = ["Prediction", "Count"]
                chart["Prediction"] = chart["Prediction"].map({
                    1: "Likely Duplicate",
                    0: "Not Duplicate"
                })
                ...
                fig = px.bar(
                    ...
                    x="Prediction",
                    y="Count",
                    title="Duplicate Prediction Summary",
                    text_auto=True
                )
    ...
    with ml_tab2:
        ...
        if anomaly_ml.empty:
            ...
            if not fa.empty and {"period", "actual_value", "final_anomaly", "metric_name"}.issubset(fa.columns):
                st.markdown("#### Metric Trend")

                chart_df = fa.copy()

                # Some files store dates as Excel serial numbers like 45700, which must not be parsed as Unix nanoseconds.
                period_raw = chart_df["period"]

                if pd.api.types.is_numeric_dtype(period_raw):
                    chart_df["period_date"] = pd.to_datetime(
                        period_raw,
                        unit="D",
                        origin="1899-12-30",
                        errors="coerce"
                    ...
                    period_text = period_raw.astype(str).str.strip()
                    period_numeric = pd.to_numeric(period_text, errors="coerce")

                    if period_numeric.notna().mean() > 0.80 and period_numeric.dropna().between(30000, 60000).all():
                        chart_df["period_date"] = pd.to_datetime(
                            period_numeric,
                            unit="D",
                            origin="1899-12-30",
                        ...
                        chart_df["period_date"] = pd.to_datetime(period_text, errors="coerce")

                if chart_df["period_date"].notna().any():
                    chart_df["period_display"] = chart_df["period_date"].dt.strftime("%Y-%m")
                else:
                    chart_df["period_display"] = chart_df["period"].astype(str)

                # If All is selected show only final anomaly rows. If one metric is selected show that metric trend.
                if selected_metric == "All":
                    chart_df = chart_df[chart_df["final_anomaly"] == 1].copy()
                    trend_title = "Anomaly Trend - Flagged Points Only"
                else:
                    chart_df = chart_df[chart_df["metric_name"].astype(str) == selected_metric].copy()
                    trend_title = f"{selected_metric} Trend"

                if chart_df.empty:
                    st.info("No trend data available for the current filter.")
                else:
                    chart_df = chart_df.sort_values("period_display")
                    chart_df["anomaly_status"] = np.where(chart_df["final_anomaly"] == 1, "Anomaly", "Normal")
                    ...
                    anomaly_points = chart_df[chart_df["final_anomaly"] == 1].copy()
                    if not anomaly_points.empty:
                        fig.add_scatter(
                            x=anomaly_points["period_display"],
                            y=anomaly_points["actual_value"],
                            mode="markers",
